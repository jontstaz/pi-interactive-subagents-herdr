 ...
 *
 * These tests exercise real herdr operations: splitting panes,
 * sending commands, reading pane output, and closing panes.
...
for (const backend of backends) {
  describe(`herdr-surface [${backend}]`, { timeout: 60_000 }, () => {
    ...
    it("keeps focus on the active surface while creating and targeting subagent surfaces", async () => {
      // Herdr has no focus-by-pane-id command; a focused split is the
      // deterministic way to place focus on the anchor pane.
      const anchor = createTrackedSurfaceSplit(env, "focus-anchor", "down", { focus: true });
